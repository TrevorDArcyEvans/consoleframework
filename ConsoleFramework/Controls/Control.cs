...
namespace ConsoleFramework.Controls
{
  ...
  public partial class Control : INotifyPropertyChanged
  {
    ...
    /// </summary>
    private static int SumWithInf(int a, int b)
    {
    ...
    /// <param name="v"></param>
    /// <returns></returns>
    private static int MinusWithInf(int v)
    {
      switch (v)
      {
        case int.MaxValue:
    ...
    internal void ResetValidity(List<Control> affectedControls)
    {
      ...
      var childrenCopy = new List<Control>(Children);
    ...
    public static Point TranslatePoint(Control source, Point point, Control dest)
    {
      ...
      else
      {
        ...
        var ancestor = FindCommonAncestor(source, dest);
    ...
    {
      var point = TranslatePoint(null, rawPoint, Parent);
    ...
    internal static bool IsPointVisible(Control control, Point point)
    {
      ...
      var layoutClip = control.LayoutClip;
      bool visible;
      if (layoutClip.IsEmpty)
      {
        var controlVirtualCanvasRect = new Rect(new Point(0, 0), control.RenderSize);
    ...
    protected static int RenderString(
      string s,
      ...
      Attr attr)
    {
      for (var i = 0; i < Math.Min(s.Length, maxWidth); i++)
      {
        char c;
    ...
    {
      var topControl = VisualTreeHelper.FindTopControlUnderMouse(this, args.GetPosition(this));
      if (topControl != null)
      {
