...
namespace ConsoleFramework.Events
{
  ...
  public sealed class FocusManager
  {
    ...
    private bool TryChangeFocusedElementTo(Control focusedControl, bool ignorePreviewHandled = false)
    {
      ...
      if (oldFocus != null)
      {
        var previewLostArgs = new KeyboardFocusChangedEventArgs(oldFocus,
      ...
      {
        var previewGotArgs = new KeyboardFocusChangedEventArgs(focusedControl,
    ...
    public void SetFocus(Control control)
    {
      ...
      var closestFocusScope = findClosestScope(control);
    ...
    public void SetFocus(Control scope, Control control)
    {
      ...
      var children = getControlsInScope(scope);
      if (children.Count == 0)
      {
      ...
      if (null != control)
      {
        ...
        {
          throw new ArgumentException("Specified control is not a child of scope or is not visible or is not focusable");
        }
    ...
    private List<Control> getControlsInScope(Control scope)
    {
      ...
      var processed = new List<Control>();
      if (scope.Focusable)
      {
      ...
      while (i < children.Count)
      {
        ...
        var nested = new List<Control>(child.Children);
      ...
      var focusableAndVisible = processed
        .Where(c => c.Visibility == Visibility.Visible && c.Focusable)
        .ToList();
    ...
    public void MoveFocusNext()
    {
      ...
      var children = getControlsInScope(_currentScope);
      if (children.Count == 0)
      {
        return;
      }

      var focusedIndex = children.FindIndex(c => c == FocusedElement);
      if (focusedIndex == -1)
      {
        SetFocus(_currentScope, null);
        return;
      ...
      {
        var child = children[(focusedIndex + 1) % children.Count];
    ...
    public void MoveFocusPrev()
    {
      ...
      var index = focusedIndex > 0 ? focusedIndex - 1 : children.Count - 1;
